...
def test_contexted_kv_attention(
    ...
    def context_attention_fwd(query, key, value, key_cache, value_cache, block_table, attn_mask):
        ...
        o, l_m = flash_paged_attention[1, 1](
    ...
    query = F.pad(query, (0, 0, 0, 0, 0, max_num_queries-k.shape[0]), "constant", 0)
    k = F.pad(k, (0, 0, 0, 0, 0, max_num_queries-k.shape[0]), "constant", 0)
    v = F.pad(v, (0, 0, 0, 0, 0, max_num_queries-v.shape[0]), "constant", 0)
    output_ref_padded = F.pad(output_ref, (0, 0, 0, 0, 0, 0, 0, max_num_queries-output_ref.shape[0]), "constant", 0)
    active_block_table = get_active_block_tables(
        block_table, torch.tensor(query_lens), torch.tensor(seq_lens), block_size, n_blocks)
