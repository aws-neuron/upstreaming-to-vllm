...
from vllm.model_executor.model_loader.neuronx_distributed import get_neuron_model, _get_model_architecture
...
class NeuronxDistributedModelRunner(NeuronModelRunner):
    ...
    def get_nxd_sampling_params(self, sampling_metadata):
        if self.model.config.neuron_config.on_device_sampling_config:
            max_topk = self.model.config.neuron_config.on_device_sampling_config.global_topk
        else:
        ...
        for index, sequenceGroupToSample in enumerate(sampling_metadata.seq_groups):
            top_k[index] = sequenceGroupToSample.sampling_params.top_k if sequenceGroupToSample.sampling_params.top_k > 0 else max_topk
        ...
        sampling_params = prepare_sampling_params(batch_size=self.scheduler_config.max_num_seqs,
    ...
    def execute_model(
        ...
        if model_input.multi_modal_kwargs.get('image') is not None:
            ...
            num_chunks = []
            has_image = []
            for multi_modal_input in model_input.multi_modal_kwargs.get('image'):
                image_tensors  = self.get_multi_modal_data_neuron(multi_modal_input.squeeze(0))
