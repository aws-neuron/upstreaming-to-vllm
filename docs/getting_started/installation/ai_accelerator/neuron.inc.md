...
AWS Neuron is the software development kit (SDK) used to run machine learning workloads on AWS Trainium and Inferentia chips. 
Both Trainium and Inferentia are powered by independent compute units called NeuronCores. This tab describes how to set up your 
environment to run vLLM on Neuron.
...
- AWS Neuron SDK 2.22/2.23

## Configure a new environment

### Launch a Trn1/Trn/2/Inf2 instance and verify Neuron dependencies
...
!!! note
    If you are looking to use the legacy [transformers-neuronx](https://github.com/aws-neuron/transformers-neuronx) library, refer to 
        [Transformers NeuronX Setup](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/libraries/transformers-neuronx/setup/index.html). 
        Note that vLLM integration for this library is no longer actively maintained. 
...
AWS Neuron maintains a fork of vLLM, which contains a fuller set of Neuron features and optimizations. To install the AWS Neuron fork, 
...
The current vLLM and Neuron integration relies on the `neuronx-distributed-inference` backend to perform most of the heavy lifting which
includes PyTorch model initialization, compilation, and runtime execution. Therefore, most [features supported on NxDI](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/libraries/nxd-inference/developer_guides/feature-guide.html) are also available via the vLLM integration. 

To configure NxDI features through the vLLM entrypoint, use the `override_neuron_config` setting. Provide the configs you want to override 
...
- Multi-LoRA serving: NxD Inference only supports loading of LoRA adapters at server startup. Dynamic loading of LoRA adapters at 
    runtime is not currently supported.
...
- `NEURON_CONTEXT_LENGTH_BUCKETS`: Bucket sizes for context encoding. (Only applicable to legacy transformers-neuronx backend).
- `NEURON_TOKEN_GEN_BUCKETS`: Bucket sizes for token generation. (Only applicable to legacy transformers-neuronx backend).
